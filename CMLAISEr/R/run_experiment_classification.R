#' Run a full experiment for comparing multiple ML algorithms using multiple
#' instances
#'
...
#' experiment. This routine builds upon [calc_instances()] and [calc_nreps_classification()],
...
#' [calc_nreps_classification()]. In short, each element of `instances` is an `instance`, i.e.,
...
#'
#' @inheritParams calc_nreps_classification
...
#'             [calc_nreps_classification()]). Loaded results (see `load.partial.results`
...
#' final_result_classification <- run_experiment(instances, algorithms)
#' summary(final_result_classification)
#' plot(final_result_classification)
#
#'
#'

run_experiment_classification <- function(instances, algorithms, d=0.5, se.max=0.001,
                                      power = 0.8, sig.level = 0.1,
                                      power.target = "mean",
                                      dif = "simple", comparisons = "all.vs.all",
                                      alternative = "two.sided", test = "t.test",
                                      method = "param",
                                      nstart = 10, nmax = 100 * length(algorithms),
                                      ...
                                      save.final.result    = NA)
{
  ...
  #                                       FUN = calc_nreps_classification,
  #                                       # Arguments for calc_nreps_classification:

...
#' experiment. This routine builds upon [calc_instances()] and [calc_nreps_regression()],
...
#' [calc_nreps_regression()]. In short, each element of `instances` is an `instance`, i.e.,
...
#'
#' @inheritParams calc_nreps_regression
...
#'
#'
#' instances <- list(list(alias = "instance1", Data = boston), list(alias = "instance2", Data = air))
#' algorithms <- c("svm", "random_forest")
#' final_result <- run_experiment_regression(instances, algorithms)
#' summary(final_result)
#' plot(final_result)

run_experiment_regression <- function(instances, algorithms, d=0.5, se.max=0.05,
                           ...
                           save.final.result    = NA)
{
  ...
  # my.results <- list()
  # for (i in 1:inst.to.use) {
  #   my.results[[i]] <- calc_nreps(instance, algorithms)
  #
  # }
